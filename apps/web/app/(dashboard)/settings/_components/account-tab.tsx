...
export function AccountTab() {
  ...
  const handleEditAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // If this profile is the default, also sync user store avatar
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      setProfiles((prev) => prev.map((p) => (p.id === updated.id ? updated : p)));
      setEditProfile(null);
      // If this profile is the default, also sync user store name
      if (updated.is_default) {
        const me = await api.updateMe({ name: editName });
        setUser(me);
      }
      // Refresh members to reflect name/avatar changes in the workspace
